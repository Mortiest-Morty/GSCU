...
class PPO():
    ...
    def __init__(self, state_dim, hidden_dim, action_dim, actor_lr, critic_lr, gamma):
        ...
        self.actor_net = PolicyNet(state_dim, self.hidden_dim, action_dim).to(device)
        self.critic_net = ValueNet(state_dim, self.hidden_dim).to(device)
        self.buffer = []
        self.counter = 0
    ...
    def select_action(self, obs, act, hidden, dim_c):
        with torch.no_grad():
            action_prob = self.actor_net(obs, act, hidden)
    ...
    def save_params(self, ckp):
        ...
        torch.save(save_dict, '../model_params/RL/ppo_params_' + str(ckp) + '.pt'
    ...
    def update(self):
        ...
        for i in range(self.ppo_update_time):
            for index in BatchSampler(SubsetRandomSampler(range(len(self.buffer))), self.batch_size, False):
                ...
                V = self.critic_net(obs_traj[index],  act_traj[index], hidden)
                delta = Gt_index - V
                advantage = delta.detach()

                action_prob = self.actor_net(obs_traj[index],  act_traj[index], hidden).gather(1, action[index])
